...
The parent rows stay phase-level, not flag-level: the flags that affect fog cost (`distJitter`, `blobJitter`, `scoutedFog`, `unexploredFog`, …) are plain per-hex conditionals inside the same loop, and wrapping each one in its own `performance.now()` call would cost more than the branch it's timing — the read would lie about what it's measuring, and would skew the very loop being measured. The sub-rows resolve this without that cost: the blob-cache split is two real timed sections (there are only two, called once each per rebuild, so timing them is free), and the per-hex splits are integer counters incremented in the branches the flags already gate — a size-of-work proxy for that branch's share of the parent's ms, not a separately measured time, but real (not fabricated) and cheap enough not to distort the loop. Toggle a flag in the panel above and read the sub-row it maps to (per the table) on the next pan/zoom; the two panels are meant to be used together.

Measured on a settlement's default view: Terrain 9.0ms (1218 drawn / 498 culled), Borders + fog 5.6ms (1625 unexplored / 19 bordered / 45 scouted), Blob cache 44.6ms (9.5ms sync / 35.1ms render) — blob cache dominated here because this sample was the settlement's very first rebuild after founding (cold GPU/shader state), not representative of steady-state cost; a warmed-up rebuild on the same view lands closer to 5ms total (see the original phase-level measurements below). On a sea-heavy panned-out world map (3965 hexes, 1652 blobs): Terrain 4.0ms (212 drawn / 937 culled), Borders + fog 10.0ms (3874 unexplored / 19 bordered / 6 scouted), Blob cache 36.4ms (6.4ms sync / 30.0ms render) — the sub-rows make it immediately visible that on this view almost the entire per-hex loop cost is `unexploredFog`'s branch (3874 of ~3965 hexes), exactly the case problem 4's background shortcut exists to skip.
