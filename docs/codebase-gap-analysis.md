...
- **Balance numbers are explicitly placeholder** (unit/building catalogues, flat 5%/level with "no Tower at all") — matches the doc's own stated limitation.
...
- **`BuildQueuePanel.vue` shows a fabricated "X / Y slots"** — `TOTAL_SLOTS = 3` is invented because no such backend concept exists, and its progress bar knowingly mis-computes percent for in-flight orders because the contract lacks a start timestamp — worth closing as a small backend contract addition.
