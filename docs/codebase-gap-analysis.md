...
- **Quarry still has no dedicated sprite in the art pack** — it renders as bare mountain terrain (`buildingArt.ts`/`textures.ts`'s fallback path), the last building type without one now that greathall/storagebuilding/bigstoragehouse/lumberjackhut/thorshrine/freyjashrine/archerybuilding/dockyard/barracks/fisherhut/sawmill have all been wired to real art.
- **Barracks has no unit-training hook of its own** — `BuildingType.Barracks` is buildable/leveling (same shape as Tower/ArcheryRange), but no `UnitDefinition` points its `RequiredBuildingType` at it: the Archery Range already owns the land combat/siege roster's gate, and there's no garrison-capacity or second training-track mechanic yet to hang a Barracks-specific one off. Same placeholder-balance caveat as the Archery Range's missing combat bonus above.
