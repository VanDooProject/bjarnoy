...
import { api, ApiError, isSettlementNotFound, isWorldNotFound } from '../api/client';
  ...
  actions: {
    ...
    async bootstrapLiveWorld() {
      ...
      try {
        ...
        if (!isWorldNotFound(err)) throw err;
        // The world resolved a moment ago (getWorld/newestWorld, just above)
        // but is already gone by the time this call landed — the same
        // "world stopped existing under us" case fetchFogMask/
        // refreshLiveSettlement recover from, just hit mid-bootstrap instead
        // of mid-poll. Fall back to the same "nothing to join yet" state a
        // deployment with no worlds at all reports, rather than leaving this
        // throw uncaught and the page stuck half-initialized.
        this.worldId = null;
        localStorage.removeItem('bjarnoy.worldId');
        this.worldJoinable = false;
        this.worldJoinableReason = 'NoWorldYet';
        return;
      }
     ...
     * `recoverFromMissingSettlement`'s own remarks) and right after a
     * mutation (`trainUnitsLive` etc.), where an ordinary failure still
     * propagates for that caller to handle, same as before.
     */
    async refreshLiveSettlement() {
      ...
      try {
        ...
        if (isSettlementNotFound(err)) {
          ...
          // start resolving again (e.g. its world got reseeded out from
          // under it) — recover instead of leaving every poll on
          // startHudSync's interval throwing an uncaught rejection forever.
    ...
    /**
     * Per-world/per-settlement caches from whichever world this session was
     * previously in — split out of `joinWorld` so `recoverFromMissingWorld`
     * below can reuse the exact same reset without also touching `ownerId`,
     * which `joinWorld` alone has reason to reassign. Deliberately
     * conservative: only state that's clearly scoped to a world or a
     * settlement within it is reset here, not app-global state like auth or
     * catalogue data.
     */
    resetPerWorldCaches() {
     ...
     * session — the backend's `world_not_found` signal (see
     * `WorldEndpoints.WorldNotFoundProblem`), as opposed to a bare 404 or a
     * transient network failure. Without this, a poll like `fetchFogMask`
     * would keep asking after the same dead id forever (the "fog covers the
     * whole map and never clears" symptom): drop the stale id and
     * re-bootstrap, the same recovery `bootstrapLiveWorld` already does for
     * itself when it finds nothing at startup. A no-op in demo mode.
     */
    async recoverFromMissingWorld() {
      if (DEMO_MODE) return;
      this.resetPerWorldCaches();
      this.worldId = null;
      localStorage.removeItem('bjarnoy.worldId');
    ...
    /**
     * Settlement counterpart to `recoverFromMissingWorld` — the backend's
     * `settlement_not_found` signal (see
     * `SettlementEndpoints.SettlementNotFoundProblem`). Deselects the dead
     * id and, via `player.enterWorld`, drops the persisted
     * `bjarnoy.settlementId` and flips onboarding back to "not founded yet"
     * for this world — the same state a brand-new player in a world they've
     * never settled sees, so the UI falls back to letting them found a new
     * settlement instead of being stuck polling one that will never come
     * back.
     */
    recoverFromMissingSettlement() {
      if (DEMO_MODE) return;
      this.selectedSettlementId = null;
      if (this.worldId) usePlayerStore().enterWorld(this.worldId, null);
    },
    /**
    ...
    async fetchFogMask() {
      ...
      try {
        ...
        if (isWorldNotFound(err)) {
          ...
          // resolving again — leaving the previous bitmap in place (this
          // catch's usual best-effort behaviour) would just poll the same
          // dead id forever. Fire-and-forget: recovery re-bootstraps this
          // store asynchronously: awaiting it here would hold
          // `maskFetchInFlight` for the whole re-bootstrap, blocking every
          // other poll on this tick's interval.
          void this.recoverFromMissingWorld();
          return;
        }
